-- Función SQL para Supabase: Prevenir reservas duplicadas
-- Ejecutar en el SQL Editor de Supabase Dashboard

-- 1. Crear índice único para prevenir duplicados en reservas confirmadas
-- En lugar de constraint, usamos un índice único parcial que es compatible con PostgreSQL
CREATE UNIQUE INDEX IF NOT EXISTS unique_confirmed_user_session_booking 
ON bookings (user_id, class_session_id) 
WHERE status = 'CONFIRMED';

-- 2. Función para obtener sesiones con contadores de reservas
...
-- 3. Función para crear reserva con validaciones atómicas
...
-- 3. Función para obtener información completa de una reserva recién creada
...
-- Comentarios de uso:
-- Para crear una reserva desde el frontend:
-- SELECT * FROM create_booking_with_validations(user_id, class_session_id);
-- Para obtener la reserva completa:
-- SELECT * FROM get_booking_with_user(booking_id_returned);
