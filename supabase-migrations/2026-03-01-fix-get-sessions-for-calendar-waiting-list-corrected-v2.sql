-- Fix get_sessions_for_calendar again
-- The issue: 'waiting' vs 'WAITING' or potentially other status mismatches
-- Solution: Make the status check case-insensitive or more robust
-- Also ensuring confirmed bookings count is case-insensitive for robustness
...
BEGIN
  ...
  -- 1. Bookings count: Robust check for 'CONFIRMED' (case insensitive)
  LEFT JOIN (
  ...
  -- Robust check for 'WAITING' (case insensitive)
  LEFT JOIN LATERAL (
    SELECT w.id
    FROM waiting_list w
    WHERE w.class_session_id = cs.id
      AND w.user_id = p_user_id
      AND UPPER(w.status) = 'WAITING'
    LIMIT 1
  ) wl ON TRUE
