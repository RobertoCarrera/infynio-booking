...
-- 1. Enforce strict expiration date checks (day precision) for both Admin and User bookings.
-- 2. Prioritize packages that expire sooner when multiple are available.
...
BEGIN
  ...
  -- FIX: Added check for v_session.schedule_date <= up.expires_at
  -- FIX: Order by expires_at ASC to prioritize sooner expiration
  ...
  WHERE up.user_id = p_target_user_id
    ...
    AND (
      ...
      -- 1. If expires_at is NULL, it never expires (valid).
      -- 2. If expires_at is set, session date must be <= expires_at.
...
BEGIN
  ...
  WITH candidates AS (
    ...
    AND (
      -- FIX: Enforce strict expiration check for ALL packages (single or monthly)
      -- If expires_at is present, session date must be <= expires_at
      c.expires_at IS NULL OR v_session.schedule_date <= c.expires_at
    )
    -- FIX: Prioritize packages that expire sooner (NULLs last), then older purchase_date as tiebreaker
