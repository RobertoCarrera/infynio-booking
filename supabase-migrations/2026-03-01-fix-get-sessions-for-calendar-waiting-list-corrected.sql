-- Fix get_sessions_for_calendar to include waiting_list status (Corrected Version)
-- This migration updates the function to include waiting_list status without breaking existing counts
-- Key fix: Preserving 'CONFIRMED' (uppercase) status check from original function
...
BEGIN
  ...
  -- 1. Bookings count (MUST use uppercase 'CONFIRMED')
  LEFT JOIN (
